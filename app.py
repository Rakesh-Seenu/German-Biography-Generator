...
def process_file_and_update_status(file):
    try:
        ...
        if df is not None:
            ...
            transkript_list_ip = extract_rows_with_sprecher(df, 'IP_')
            transkript_list_int = extract_rows_with_sprecher(df, 'INT_')
            ...
            socketio.emit('update', {'file': file.filename, 'status': 'Processing'})

            initial_biography = summarizer.generate_biography(transcript_data)
            
            output_pdf_path = os.path.join(output_directory, file.filename.replace('.csv', '.pdf'))
            save_text_to_pdf(initial_biography.strip(), output_pdf_path)

            socketio.emit('update', {'file': file.filename, 'status': 'Completed'})
        else:
            socketio.emit('update', {'file': file.filename, 'status': 'Failed'})
    except Exception as e:
...
def home():
    ...
    if request.method == 'POST':
        files = request.files.getlist('files')  # Getting multiple files from input
        if files:
            for file in files:
                if file and allowed_file(file.filename):
                    status_dict[file.filename] = "Waiting"
                    thread = threading.Thread(target=process_file_and_update_status, args=(file,))
                    thread.start()
        else:
            flash("No files selected or invalid file type.")
